...
class FriendRequestNotificationFactory(context:Context):MobileNotificationFactory(context) {
    ...
    private fun buildNotification(friendRequestNotificationModel: FriendRequestNotificationModel)
    :NotificationCompat.Builder{

        val intent = Intent(context, NewNotificationToLaunchAppBroadcast::class.java)
        intent.action = "newNotification"
        intent.putExtra(ActivityIntentsExtrasKeys.IntentType,AcivityIntentsTypes.FriendRequest.name)
        intent.putExtra(ActivityIntentsExtrasKeys.userName,
        friendRequestNotificationModel.requestSenderUserData?.userUserName)
        ...
        val notificationBuilder = NotificationCompat.Builder(context, NotificationChannelsIds.HighImportanceNotificationChannel )
            .setContentTitle(context.resources.getString(R.string.FriendRequest))
            .setContentText(context.resources.getString(R.string.YouHaveFriendRequestFrom)+
                    " ${friendRequestNotificationModel.requestSenderUserData?.userFullName}")
    ...
    fun sendNotification(friendRequestNotificationModel: FriendRequestNotificationModel){

        var notification = buildNotification(friendRequestNotificationModel).build()
        var notificationId = mobileNotificationsUtilities.newId(NotificationTypes.FriendRequest)
